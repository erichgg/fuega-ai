...
/**
 * GET /api/campfires/:id/chat
 * Get recent messages OR open an SSE stream.
...
export async function GET(req: Request, context: RouteContext) {
  ...
  if (stream) {
    ...
    const readable = new ReadableStream({
      start(controller) {
        ...
        const unsubscribe = subscribeToCampfire(campfireId, (msg: ChatMessage) => {
  ...
  // Regular fetch
  try {
    const messages = await getRecentMessages(campfireId, limit, before);
