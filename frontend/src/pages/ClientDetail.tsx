...
const timelineIcon = (status: string) => {
  ...
  return <Circle className="w-4 h-4 text-chispa-text-muted" />;
};
...
export default function ClientDetail() {
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="flex items-center gap-4 mb-4 text-sm text-chispa-text-secondary">
        <div className="w-10 h-10 rounded-xl bg-chispa-orange/10 flex items-center justify-center">
          <span className="text-lg font-bold text-chispa-orange">{client.name?.charAt(0)}</span>
        </div>
        ...
        <ChartCard title="Deliverables" subtitle={`${deliverables.length} items`}>
          {deliverables.length === 0 ? (
            ...
            <div className="relative pl-6">
              {/* Timeline line */}
              <div className="absolute left-[7px] top-2 bottom-2 w-px bg-chispa-border" />
              <div className="space-y-1">
                {deliverables.map((d: any) => (
                  <div key={d.id} className="relative flex items-start gap-2 py-1.5 px-2 rounded-lg hover:bg-chispa-card-hover transition-colors">
                    {/* Timeline dot */}
                    <div className="absolute -left-6 top-2.5 flex items-center justify-center">
                      {timelineIcon(d.status)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-chispa-text-primary truncate">{d.title}</p>
                      <p className="text-[10px] text-chispa-text-muted">{d.service_type} {d.due_date ? `\u00b7 Due ${d.due_date}` : ''}</p>
                    </div>
          ...
          {invoices.length === 0 ? (
            ...
            <div className="space-y-1">
              {invoices.map((inv: any) => (
                <div key={inv.id} className="flex items-center gap-2 py-1.5 px-2 rounded-lg hover:bg-chispa-card-hover transition-colors">
                  <DollarSign className="w-4 h-4 text-chispa-text-muted flex-shrink-0" />
                  <div className="flex-1">
                    <p className="num text-sm text-chispa-text-primary">${inv.amount_usd}</p>
                    <p className="text-[10px] text-chispa-text-muted">Period: {inv.period_start}</p>
                  </div>
