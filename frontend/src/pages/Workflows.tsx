...
const stepStatusIcon: Record<string, React.ReactNode> = {
  ...
  pending: <Clock className="w-3.5 h-3.5 text-fuega-text-muted" />,
  failed: <XCircle className="w-3.5 h-3.5 text-red-400" />,
  awaiting_approval: <AlertTriangle className="w-3.5 h-3.5 text-fuega-orange" />,
};
      ...
      {approvals > 0 && (
        <div className="mb-3 bg-fuega-orange/5 border-l-2 border-fuega-orange rounded-r-lg p-3">
          ...
          {workflows.filter(w => w.status === 'paused_for_approval').map(wf => {
            ...
            return (
              <div key={wf.id} className="bg-fuega-card border border-fuega-border rounded-lg mb-2 overflow-hidden">
                {/* Header */}
                <div className="flex items-center justify-between p-2.5">
                  <div>
                    <p className="text-[12px] font-medium text-fuega-text-primary">
                  ...
                  <div className="border-t border-fuega-border/50 p-2.5 space-y-2 max-h-64 overflow-y-auto">
                    ...
                    {summaryItems.map((item, i) => (
                      <div key={i} className="bg-fuega-surface rounded-lg p-2 border border-fuega-border/30">
                        ...
                        <pre className="text-[10px] text-fuega-text-secondary whitespace-pre-wrap font-mono leading-relaxed">{item.content}</pre>
                      </div>
                    ))}
                  </div>
                )}
      ...
      {tab === 'pipelines' && definitions && (
        <div className="space-y-3">
          {Object.entries(definitions).map(([key, wf]: [string, any]) => (
            <div key={key} className="bg-fuega-card border border-fuega-border rounded-lg overflow-hidden">
              <div className="flex items-center gap-3 px-3 py-2.5">
                <span className="text-base">{WORKFLOW_ICONS[key] || '\u2699\uFE0F'}</span>
                <div className="flex-1">
                  <p className="text-[12px] font-semibold text-fuega-text-primary">{wf.name}</p>
                  <p className="text-[10px] text-fuega-text-muted">{wf.description}</p>
                </div>
              ...
              {expandedDef === key && (
                <div className="px-3 pb-3 border-t border-fuega-border/50 pt-2 animate-slideDown">
                  <div className="relative pl-6">
                    <div className="absolute left-[7px] top-1 bottom-1 w-0.5 bg-fuega-border" />
                    {wf.steps.map((step: any, i: number) => (
                      <div key={step.id} className="relative flex items-start gap-3 py-1.5">
                        <div className="absolute -left-6 top-2 z-10 bg-fuega-card">
                          <div className="w-4 h-4 rounded-full border-2 border-fuega-border bg-fuega-card flex items-center justify-center text-[8px] font-bold text-fuega-text-muted">
                            {i + 1}
                          </div>
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            <span className="text-[12px] text-fuega-text-primary font-medium">{step.id.replace(/_/g, ' ')}</span>
                            {step.agent && (
      ...
      {tab === 'steps' && definitions && (
        <div>
          ...
          <div className="space-y-3">
            {Object.entries(definitions).map(([key, wf]: [string, any]) => (
              <div key={key}>
                <h3 className="text-[11px] font-bold uppercase tracking-wider text-fuega-text-muted mb-1.5 flex items-center gap-2">
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {wf.steps.filter((s: any) => s.agent).map((step: any) => {
                        ...
                        return (
                          <tr key={step.id} className="border-b border-fuega-border/30 hover:bg-fuega-card-hover transition-colors">
                            <td className="px-3 py-1.5">
                              <p className="text-[12px] font-medium text-fuega-text-primary">{step.id.replace(/_/g, ' ')}</p>
                              <p className="text-[10px] text-fuega-text-muted">{step.description}</p>
                            </td>
                            <td className="px-3 py-1.5">
                              <span className="flex items-center gap-1 text-[10px] text-fuega-orange">
                                <Bot className="w-2.5 h-2.5" /> {step.agent_name}
                              </span>
                            </td>
                            <td className="px-3 py-1.5">
                              {step.model && <span className="text-[10px] text-fuega-purple">{modelShort(step.model)}</span>}
                            </td>
                            <td className="px-3 py-1.5">
                              <button
          ...
          {filteredWorkflows.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {filteredWorkflows.map(wf => {
                ...
                return (
                  <div key={wf.id} className="border border-fuega-border rounded-lg overflow-hidden">
                    <button
                      onClick={() => handleExpand(wf.id)}
                      className="w-full flex items-center gap-3 px-3 py-2 hover:bg-fuega-card-hover transition-colors"
                    >
                      {isExpanded ? <ChevronDown className="w-3.5 h-3.5 text-fuega-text-muted" /> : <ChevronRight className="w-3.5 h-3.5 text-fuega-text-muted" />}
                      <span className="text-base">{WORKFLOW_ICONS[wf.workflow_name] || '\u2699\uFE0F'}</span>
                      <div className="flex-1 text-left">
                        <span className="text-[12px] font-medium text-fuega-text-primary">{definitions?.[wf.workflow_name]?.name || wf.workflow_name}</span>
                        <span className="text-[10px] text-fuega-text-muted ml-2">#{wf.id}</span>
                      </div>
                      <span className="text-[10px] text-fuega-text-muted num">${runCost.toFixed(3)}</span>
                      <Badge variant={wf.status} />
                      <span className="text-[10px] text-fuega-text-muted">
                        {wf.started_at ? new Date(wf.started_at).toLocaleString('en', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : ''}
                      </span>
                    </button>

                    {isExpanded && wf.steps && (
                      <div className="px-3 pb-3 pt-1 border-t border-fuega-border/50 animate-slideDown">
                        <div className="relative pl-6">
                          <div className="absolute left-[7px] top-1 bottom-1 w-0.5 bg-fuega-border" />
                          {wf.steps.map((step: any) => (
                            <div key={step.step_id} className="relative flex items-start gap-2 py-1.5">
                              ...
                              <div className="flex-1 ml-1">
                                <div className="flex items-center gap-2">
                                  <span className="text-[12px] text-fuega-text-primary font-medium">{(step.step_id || 'unknown').replace(/_/g, ' ')}</span>
                                  {step.agent_slug && <span className="text-[10px] text-fuega-text-muted">({step.agent_slug.replace(/_/g, ' ')})</span>}
                                </div>
