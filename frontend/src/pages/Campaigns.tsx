...
export default function Campaigns() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setAds(a);
      setEmails(e);
    }).catch(() => toast.error(t('common:errors.failedToLoad', { resource: t('campaigns:title').toLowerCase() }) + ' ' + t('common:errors.backendCheck')))
  ...
  const adColumns: Column<any>[] = [
    {
      key: 'name',
      label: t('campaigns:adColumns.campaign'),
      sortable: true,
    ...
    {
      key: 'platform',
      label: t('campaigns:adColumns.platform'),
      sortable: true,
    ...
    {
      key: 'status',
      label: t('campaigns:adColumns.status'),
      render: (row) => <Badge variant={row.status} />,
    },
    {
      key: 'spend',
      label: t('campaigns:adColumns.spend'),
      sortable: true,
    ...
    {
      key: 'clicks',
      label: t('campaigns:adColumns.clicks'),
      sortable: true,
    ...
    {
      key: 'ctr',
      label: t('campaigns:adColumns.ctr'),
      sortable: true,
    ...
    {
      key: 'cpc',
      label: t('campaigns:adColumns.cpc'),
      sortable: true,
    ...
    {
      key: 'roas',
      label: t('campaigns:adColumns.roas'),
      sortable: true,
  ...
  const emailColumns: Column<any>[] = [
    {
      key: 'name',
      label: t('campaigns:emailColumns.campaign'),
      sortable: true,
      getValue: (row) => row.name,
      render: (row) => <span className="text-[12px] text-fuega-text-primary font-medium">{row.name}</span>,
    },
    ...
    {
      key: 'open_rate',
      label: t('campaigns:emailColumns.openRate'),
      sortable: true,
    ...
    {
      key: 'click_rate',
      label: t('campaigns:emailColumns.clickRate'),
      sortable: true,
  ...
  const adPerformanceData = ads.length > 0 ? {
    ...
    datasets: [
      ...
      {
        label: t('campaigns:impressionsLabel'),
  ...
  const campaignTabs = [
    { key: 'ads', label: t('campaigns:tabs.paidAds'), count: ads.length },
    { key: 'email', label: t('campaigns:tabs.email'), count: emails.length },
  ];
  ...
  return (
    <div className="animate-fadeIn">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-3">
        <StatCard label={t('campaigns:stats.totalAdSpend')} value={<span className="num">${totalAdSpend.toFixed(0)}</span>} icon={<DollarSign className="w-5 h-5" />} color="orange" />
        <StatCard label={t('campaigns:stats.totalClicks')} value={<span className="num">{totalClicks.toLocaleString()}</span>} icon={<MousePointerClick className="w-5 h-5" />} color="teal" />
        <StatCard label={t('campaigns:stats.avgRoas')} value={<span className="num">{avgRoas.toFixed(1)}x</span>} icon={<TrendingUp className="w-5 h-5" />} color="indigo" />
        <StatCard label={t('campaigns:stats.emailCampaigns')} value={emails.length} icon={<Mail className="w-5 h-5" />} color="pink" />
      </div>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        {searchQuery && (
          <span className="text-[11px] text-fuega-text-muted">{(tab === 'ads' ? filteredAds.length : filteredEmails.length) !== 1 ? t('common:pagination.results', { count: tab === 'ads' ? filteredAds.length : filteredEmails.length }) : t('common:pagination.result', { count: tab === 'ads' ? filteredAds.length : filteredEmails.length })}</span>
        )}
      </div>
      ...
      {tab === 'ads' && (
        ads.length === 0 ? (
          <EmptyState title={t('campaigns:empty.noAds')} description={t('campaigns:empty.noAdsDesc')} />
        ) : (
          <>
            {adPerformanceData && (
              <ChartCard title={t('campaigns:adPerformance')} subtitle={t('campaigns:adPerformanceSubtitle')} className="mb-3">
                <div className="h-64">
            ...
            <DataTable
              ...
              compact
              emptyMessage={t('campaigns:empty.noAdsMatch')}
            />
          </>
        )
      )}
      ...
      {tab === 'email' && (
        emails.length === 0 ? (
          <EmptyState title={t('campaigns:empty.noEmails')} description={t('campaigns:empty.noEmailsDesc')} />
        ) : (
          <DataTable
            ...
            getRowKey={(row) => row.id}
            compact
            emptyMessage={t('campaigns:empty.noEmailsMatch')}
          />
        )
      )}
    </div>
