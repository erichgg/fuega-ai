...
describe("seed data loaded correctly", () => {
  it("has 4 users (system + 3 test)", async () => {
    ...
    expect(parseInt((rows[0] as any).count)).toBe(4);
  });
  ...
  it("has 5 categories", async () => {
    ...
    expect(parseInt((rows[0] as any).count)).toBe(5);
  });
  ...
  it("has 2 communities", async () => {
    ...
    expect(parseInt((rows[0] as any).count)).toBe(2);
  });
  ...
  it("has 3 posts", async () => {
    ...
    expect(parseInt((rows[0] as any).count)).toBe(3);
  });
...
describe("user -> posts relationship", () => {
  it("user_1 has 1 post in test_tech", async () => {
    ...
    expect(parseInt((rows[0] as any).count)).toBe(1);
  });
  ...
  it("posts correctly reference their community", async () => {
    const { rows } = await db.query<{ campfire_id: string }>(
      `SELECT campfire_id FROM posts WHERE id = $1`,
      [TEST_IDS.post1]
    );
    expect((rows[0] as any).campfire_id).toBe(TEST_IDS.campfireTestTech);
  });
});
...
describe("community -> posts relationship", () => {
  it("test_tech has 2 posts", async () => {
    const { rows } = await db.query<{ count: string }>(
      `SELECT COUNT(*) as count FROM posts WHERE campfire_id = $1`,
...
describe("comment threading", () => {
  it("comment_1 is a top-level comment (no parent)", async () => {
    ...
    expect((rows[0] as any).parent_id).toBeNull();
  ...
  it("comment_2 is a reply to comment_1", async () => {
    ...
    expect((rows[0] as any).parent_id).toBe(TEST_IDS.comment1);
    expect((rows[0] as any).depth).toBe(1);
  });
  ...
  it("comment_3 is a top-level comment on a different post", async () => {
    ...
    expect((rows[0] as any).parent_id).toBeNull();
    expect((rows[0] as any).depth).toBe(0);
    expect((rows[0] as any).post_id).toBe(TEST_IDS.post2);
  });
  ...
  it("can query full thread for a post with nested comments", async () => {
    ...
    expect((rows[0] as any).parent_id).toBeNull();
    // Second is the reply
    expect((rows[1] as any).parent_id).toBe((rows[0] as any).id);
  });
  ...
  it("can insert deeply nested comments (up to depth 99)", async () => {
    ...
    expect((rows[0] as any).depth).toBe(5);
...
describe("vote anonymization", () => {
  it("new votes are not anonymized by default", async () => {
    ...
    expect((rows[0] as any).user_id).toBe(TEST_IDS.testUser1);
  ...
  it("anonymized votes still count for totals", async () => {
    ...
    expect(parseInt((rows[0] as any).total)).toBe(0); // 1 + (-1) = 0
  ...
  it("can find non-anonymized votes older than threshold (for anonymization job)", async () => {
    ...
    expect(parseInt((rows[0] as any).count)).toBeGreaterThanOrEqual(1);
...
describe("soft delete behavior", () => {
  ...
  it("soft-deleted comments still exist in DB", async () => {
    ...
    expect((children[0] as any).id).toBe(TEST_IDS.comment2);
...
describe("moderation log relationships", () => {
  it("mod logs reference correct communities", async () => {
    const { rows } = await db.query(
      `SELECT ml.id, ml.campfire_id, c.name as campfire_name
       FROM moderation_log ml
       JOIN communities c ON c.id = ml.campfire_id
       ORDER BY ml.created_at`
    );
    expect(rows.length).toBe(3);
    // First two are in test_tech, third in demo_science
    expect((rows[0] as any).campfire_name).toBe("test_tech");
    expect((rows[1] as any).campfire_name).toBe("test_tech");
    expect((rows[2] as any).campfire_name).toBe("demo_science");
  });
  ...
  it("mod logs reference correct authors", async () => {
    ...
    expect(rows.length).toBe(3);
    expect((rows[0] as any).username).toBe("test_user_1");
    expect((rows[1] as any).username).toBe("test_user_2");
    expect((rows[2] as any).username).toBe("test_user_2");
  });
  ...
  it("can create appeals linked to moderation log entries", async () => {
    ...
    expect((rows[0] as any).appeal_text).toBe("I disagree with this decision");
    expect((rows[0] as any).decision).toBe("approved");
...
describe("community membership roles", () => {
  it("test_user_1 is admin of test_tech", async () => {
    const { rows } = await db.query(
      ...
      [TEST_IDS.testUser1, TEST_IDS.campfireTestTech]
    );
  ...
  it("test_user_2 is member of test_tech and admin of demo_science", async () => {
    const { rows } = await db.query(
      `SELECT cm.role, c.name as community
       ...
       JOIN communities c ON c.id = cm.campfire_id
       WHERE cm.user_id = $1
       ORDER BY c.name`,
      [TEST_IDS.testUser2]
    );
    expect(rows.length).toBe(2);
    expect((rows[0] as any).campfire).toBe("demo_science");
    expect((rows[0] as any).role).toBe("admin");
    expect((rows[1] as any).campfire).toBe("test_tech");
    expect((rows[1] as any).role).toBe("member");
  });
...
describe("AI prompt history", () => {
  ...
  it("prompt history references correct entities", async () => {
    ...
    expect((rows[0] as any).entity_type).toBe("community");
    expect((rows[0] as any).entity_id).toBe(TEST_IDS.campfireTestTech);
    expect((rows[0] as any).version).toBe(1);
    expect((rows[1] as any).entity_type).toBe("community");
    expect((rows[1] as any).entity_id).toBe(TEST_IDS.campfireDemoScience);
  });
});
...
describe("proposals and proposal votes", () => {
  it("can create a proposal and vote on it", async () => {
    ...
    const { rows: propRows } = await db.query<{ id: string }>(
      `INSERT INTO proposals (id, campfire_id, proposal_type, title, description, proposed_changes, created_by, discussion_ends_at, voting_ends_at)
