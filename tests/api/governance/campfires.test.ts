...
import {
  ...
  updateCampfireSchema,
  listCampfiresSchema,
} from "@/lib/validation/campfires";

let db: PGlite;

describe("campfires service", () => {
  beforeAll(async () => {
    db = await getTestDb();
  });
  ...
  describe("createCampfire", () => {
    ...
    it("should auto-join creator as admin", async () => {
      const community = await createCampfire(
        {
          name: "admin_test",
  ...
  describe("getCampfireById", () => {
    it("should return a community by ID", async () => {
      const result = await getCampfireById(TEST_IDS.campfireTestTech);
      expect(result).toBeDefined();
    ...
    it("should return null for non-existent ID", async () => {
      const result = await getCampfireById("99999999-9999-9999-9999-999999999999");
    ...
    it("should return null for soft-deleted community", async () => {
      ...
      const result = await getCampfireById(TEST_IDS.campfireDemoScience);
      expect(result).toBeNull();
  ...
  describe("getCampfireByName", () => {
    it("should return a community by name", async () => {
      const result = await getCampfireByName("test_tech");
      expect(result).toBeDefined();
      expect(result!.id).toBe(TEST_IDS.campfireTestTech);
    });

    it("should return null for non-existent name", async () => {
      const result = await getCampfireByName("does_not_exist");
      expect(result).toBeNull();
    });
  });
  ...
  describe("listCampfires", () => {
    it("should list all communities", async () => {
      const result = await listCampfires({
        sort: "members",
        limit: 25,
        offset: 0,
      });
    ...
    it("should filter by category", async () => {
      ...
      // category filtering not supported in current schema
    ...
    it("should paginate correctly", async () => {
      const all = await listCampfires({
        sort: "members",
        limit: 25,
        offset: 0,
      });

      const page1 = await listCampfires({
        sort: "members",
        limit: 1,
        offset: 0,
      });
    ...
    it("should exclude banned communities", async () => {
      ...
      const bannedIds = result.map((c: { id: string }) => c.id);
      expect(bannedIds).not.toContain(TEST_IDS.campfireDemoScience);
  ...
  describe("updateCampfire", () => {
    it("should update community description (admin only)", async () => {
      const result = await updateCampfire(
  ...
  describe("joinCampfire", () => {
    it("should allow a user to join a community", async () => {
      ...
      expect(membership.campfire_id).toBe(TEST_IDS.campfireDemoScience);

      // Clean up
      await db.exec(
        `DELETE FROM community_memberships
         WHERE user_id = '${TEST_IDS.testUser1}'
         AND community_id = '${TEST_IDS.campfireDemoScience}'`
      );
      await db.exec(
    ...
    it("should increment member count on join", async () => {
      const before = await getCampfireById(TEST_IDS.campfireDemoScience);
      const beforeCount = before!.member_count;

      await joinCampfire(TEST_IDS.campfireDemoScience, TEST_IDS.testUser1);

      const after = await getCampfireById(TEST_IDS.campfireDemoScience);
    ...
    it("should reject double-join", async () => {
      ...
      await expect(
        joinCampfire(TEST_IDS.campfireTestTech, TEST_IDS.testUser2)
      ).rejects.toThrow(ServiceError);

      try {
        await joinCampfire(TEST_IDS.campfireTestTech, TEST_IDS.testUser2);
      } catch (err) {
    ...
    it("should reject joining a non-existent community", async () => {
      await expect(
        joinCampfire("99999999-9999-9999-9999-999999999999", TEST_IDS.testUser1)
      ).rejects.toThrow(ServiceError);
    });
    ...
    it("should reject joining a banned community", async () => {
      ...
      await expect(
        joinCampfire(TEST_IDS.campfireDemoScience, TEST_IDS.testUser1)
      ).rejects.toThrow(ServiceError);
  ...
  describe("leaveCampfire", () => {
    it("should allow a member to leave", async () => {
      // testUser2 is a member of test_tech
      await leaveCampfire(TEST_IDS.campfireTestTech, TEST_IDS.testUser2);

      const membership = await getMembership(TEST_IDS.testUser2, TEST_IDS.campfireTestTech);
      expect(membership).toBeNull();

      // Restore
      await db.exec(
        `INSERT INTO community_memberships (user_id, community_id, role)
         VALUES ('${TEST_IDS.testUser2}', '${TEST_IDS.campfireTestTech}', 'member')`
      );
      await db.exec(
    ...
    it("should decrement member count on leave", async () => {
      const before = await getCampfireById(TEST_IDS.campfireTestTech);
      const beforeCount = before!.member_count;

      await leaveCampfire(TEST_IDS.campfireTestTech, TEST_IDS.testUser2);

      const after = await getCampfireById(TEST_IDS.campfireTestTech);
      expect(after!.member_count).toBe(beforeCount - 1);
    ...
    it("should reject leaving if not a member", async () => {
      await expect(
        leaveCampfire(TEST_IDS.campfireDemoScience, TEST_IDS.testUser1)
      ).rejects.toThrow(ServiceError);

      try {
        await leaveCampfire(TEST_IDS.campfireDemoScience, TEST_IDS.testUser1);
      } catch (err) {
    ...
    it("should reject last admin leaving", async () => {
      ...
      await expect(
        leaveCampfire(TEST_IDS.campfireTestTech, TEST_IDS.testUser1)
      ).rejects.toThrow(ServiceError);

      try {
        await leaveCampfire(TEST_IDS.campfireTestTech, TEST_IDS.testUser1);
      } catch (err) {
    ...
    it("should reject leaving non-existent community", async () => {
      await expect(
        leaveCampfire("99999999-9999-9999-9999-999999999999", TEST_IDS.testUser1)
      ).rejects.toThrow(ServiceError);
    });
  });
  ...
  describe("getMembership", () => {
    it("should return membership for an existing member", async () => {
      const result = await getMembership(TEST_IDS.testUser1, TEST_IDS.campfireTestTech);
      expect(result).toBeDefined();
      expect(result!.role).toBe("admin");
    });

    it("should return null for non-members", async () => {
      const result = await getMembership(TEST_IDS.testUser1, TEST_IDS.campfireDemoScience);
      expect(result).toBeNull();
    });
  });
  ...
  describe("isAdmin", () => {
    it("should return true for admins", async () => {
      const result = await isAdmin(TEST_IDS.testUser1, TEST_IDS.campfireTestTech);
      expect(result).toBe(true);
    });

    it("should return false for regular members", async () => {
      const result = await isAdmin(TEST_IDS.testUser2, TEST_IDS.campfireTestTech);
      expect(result).toBe(false);
    });

    it("should return false for non-members", async () => {
      const result = await isAdmin(TEST_IDS.testUser1, TEST_IDS.campfireDemoScience);
      expect(result).toBe(false);
    });
  });
});
...
describe("campfire validation schemas", () => {

  describe("createCampfireSchema", () => {
    it("should accept valid input", () => {
      const result = createCampfireSchema.safeParse({
        name: "valid_name",
  ...
  describe("updateCampfireSchema", () => {
    it("should accept partial updates", () => {
      const result = updateCampfireSchema.safeParse({
  ...
  describe("listCampfiresSchema", () => {
    it("should accept valid sort options", () => {
      for (const sort of ["members", "activity", "created_at"]) {
        const result = listCampfiresSchema.safeParse({ sort });
        expect(result.success).toBe(true);
      }
    });

    it("should default sort to members", () => {
      const result = listCampfiresSchema.safeParse({});
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    it("should reject invalid sort options", () => {
      const result = listCampfiresSchema.safeParse({ sort: "invalid" });
      expect(result.success).toBe(false);
    });
  });
});
